...
{\setstretch{1.5}
\par 本文我们主要研究图像去雾领域具有代表意义的去雾算法，暗通道法、DehazeNet、MSCNN和AOD-Net。其中，暗通道法代表
}
\end{abstract}
...
\par In this paper, we present a comprehensive study and evaluation of some seminar dehazing algorithms for a single image: dark channel prior(DCP), DehazeNet, MSCNN, and AOD-Net. DCP represents traditional ways of image dehazing, i.e., prior-based haze removal methods, while the other three are well developed learning based image dehazing methods. DehazeNet is a first-in-class approach to combine Convolutional Neural Network (CNN) with traditional method for haze removal. Similarly, MSCNN generalized the idea in DehazeNet to predict the transmission map with a single feed-forward computation. AOD-Net, on the other hand, employs a much simpler full-convolutional network architecture.  

Image quality assessment (IQA) is another big topic discussed in this paper. We utilized full reference (FR) and no reference (NR) IQA methods to evaluate the performances of aforementioned algorithms. 
\end{englishabstract}
\newpage
...
% \setstretch{1.5} could be used to change the line spacing here, line spacing = baselineskip(10.5) * setstretch
...
%brisque \url{https://www.learnopencv.com/image-quality-assessment-brisque/}
